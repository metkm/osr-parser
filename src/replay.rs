...
impl Replay {
    pub fn read(path: &str) -> Result<Self, ReplayError> {
        ...
        Ok(Self {
            ...
            beatmap_md5: read_string(&mut p, &mut content)
                .unwrap_or_else(|_| "Can't read beatmap md5!".to_string()),
            username: read_string(&mut p, &mut content)
                .unwrap_or_else(|_| "Can't read username!".to_string()),
            replay_md5: read_string(&mut p, &mut content)
                .unwrap_or_else(|_| "Can't read replay md5!".to_string()),
            ...
            life_bar: read_string(&mut p, &mut content).unwrap_or_else(|_| "".to_string()),
